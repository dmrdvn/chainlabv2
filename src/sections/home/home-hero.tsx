...
export function HomeHero({ sx, ...other }: BoxProps) {
  ...
  const renderText = () => (
    <m.div {...motionProps}>
      ...
      >
        {`ChainLab's AI-first approach removes technical barriers,\n enabling developers of all skill levels to contribute to the decentralized ecosystem.`}
      </Typography>
    </m.div>
  );
        ...
        {['ethereum', 'base', 'bsc', 'opbnb'].map((platform) => (
          <m.div {...motionProps} key={platform}>
            <Box
              component="img"
              alt={platform}
              src={`${CONFIG.assetsDir}/assets/icons/platforms/ic-${platform}.png`}
              sx={[
                {
                  width: 24,
                  height: 24,
